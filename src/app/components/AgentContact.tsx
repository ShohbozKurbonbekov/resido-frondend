...
const AgentContact: React.FC<AgentContactProp> = React.memo(
  ({ contactData }) => {
    const { authmember } = useGlobals();
    const [messageInput, setMessageInput] =
      useState<MessageInput>(initialState);
    const { avatar, name, phone, id: _id, role } = contactData;
    ...
    const defaultImage =
      ...
      role === MemberType.USER
        ? defaultUserAvatar
        : defaultAgencyAvatar;
    const imgUrl = avatar ? `${serverAPI}/${avatar}` : defaultImage;
