...
export default function PaymentInfoContent({ id }: PaymentInfoContentType) {
  const [currentPlan, setCurrentPlan] = useState<null | TarrifOutputType>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const result = await member.getTariff(id);
        setCurrentPlan(result);
      } catch (error) {
        console.log("Error in getting PaymentInfoContent: ", error);
  ...
  const onSubmit = useCallback(
    ...
    [form, setAuthMember, currentPlan, id],
  );
